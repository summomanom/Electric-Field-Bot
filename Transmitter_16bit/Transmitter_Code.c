...
#define FREQ 15500L //Hz

#define BITWIDTH 5 //ms
#define DELAY 16    //bits
...
void main (void)
{
	...
	while(1)
	{
		...
		if(Vx>VxMidHigh && Vy>VyMidHigh)  //forward right
		{
			backward_L=0;
			backward_R=0;
			forward_L=(int)(((Vx-VxMid)/VxMid)*10);
			forward_R=(int)((((Vx-VxMid)/VxMid)*10)/2.0);
		}
		else if(Vx>VxMidHigh && Vy<VyMidLow) //forward left
		{
			backward_L=0;
			backward_R=0;
			forward_L=(int)((((Vx-VxMid)/VxMid)*10)/2.0);
			forward_R=(int)(((Vx-VxMid)/VxMid)*10);	
		}
		...
		{
			backward_L=(int)(((VxMid-Vx)/VxMid)*10);
			backward_R=(int)((((VxMid-Vx)/VxMid)*10)/2.0);
			forward_L=0;
			forward_R=0;	
		}
		else if(Vx<VxMidLow && Vy<VyMidLow)  //back left 110 1 010011001001
		{
			backward_L=(int)((((VxMid-Vx)/VxMid)*10)/2.0);
			backward_R=(int)(((VxMid-Vx)/VxMid)*10);
			forward_L=0;
			forward_R=0;
		}
		else if(Vx<VxMidLow && Vy>VyMidLow && Vy<VyMidHigh) //backward 110 0 0000 1 1010 00
		{
			backward_L=(int)(((VxMid-Vx)/VxMid)*10);
			backward_R=(int)(((VxMid-Vx)/VxMid)*10);
			forward_L=0;
		...
		else if(Vx<VxMidHigh && Vx>VxMidLow && Vy<VyMidLow) // left
		{
			...
			backward_R=0;
			forward_L=0;
			forward_R=(int)(((VyMid-Vy)/VyMid)*10);		
		}
