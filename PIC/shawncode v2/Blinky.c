...
#define FREQ2 10000L // 1000Hz or 0.001 seconds interrupt rate
...
{	
	...
	while(IEC0bits.T2IE ==1)
	{	
		voltage = ADCRead(5)*3.3/1023.0;
		if(voltage<0.4)
		{
			IEC0bits.T2IE = 0;
			FREQ1 = 16*10/(count * 0.0001*13);
			SetupTimer1();
			command[0] = '1';
			command[1] = '1';
			i = 2;
